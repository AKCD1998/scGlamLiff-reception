...
function TopicMetric({ title, section }) {
  const availability = section?.availability || section?.status || "unavailable";
  const messages = [
    section?.summary,
    section?.explanation,
    section?.reason,
    section?.note,
    Array.isArray(section?.missingRequirements) && section.missingRequirements.length
      ? `ยังขาด: ${section.missingRequirements.join(", ")}`
      : "",
  ].filter(Boolean);

  return (
    <article className={`kpi-unavailable-card ${metricToneClass(availability)}`}>
      <div className="kpi-summary-top">
        <p>{title}</p>
        <AvailabilityBadge availability={availability} />
      </div>
      <strong>{messages[0] || "ยังคำนวณไม่ได้อย่างโปร่งใส"}</strong>
...
export default function KpiDashboardPage() {
  ...
  const topicSections = [
    sections.free_scan_conversion,
    sections.upsell_conversion,
    sections.revenue_mix,
  ].filter(Boolean);
  const hasReadableTopicData = topicSections.some(
    (section) => section?.status === "available" || section?.status === "fallback"
  );
        ...
        <>
          <div className="kpi-dashboard-main">
            ...
            <div className="kpi-two-column kpi-two-column--retention-unavailable">
              ...
              <article className="panel kpi-panel--unavailable-compact">
                <div className="panel-title">
                  <span>{hasReadableTopicData ? "หัวข้อ KPI เพิ่มเติม" : "ตัวชี้วัดที่ยังไม่มีข้อมูลพอ"}</span>
                  <strong>{hasReadableTopicData ? "ตามความพร้อมของข้อมูล" : "แสดงแบบโปร่งใส"}</strong>
                </div>
                <div className="kpi-unavailable-grid">
                  <TopicMetric title="แปลงจากสแกนผิวฟรี" section={sections.free_scan_conversion || { availability: "unavailable" }} />
                  <TopicMetric title="Upsell ผลิตภัณฑ์" section={sections.upsell_conversion || { availability: "unavailable" }} />
                  <TopicMetric title="Revenue mix บริการ vs สินค้า" section={sections.revenue_mix || { availability: "unavailable" }} />
                </div>
              </article>
            </div>
          </div>
          ...
          {assumptions.length ? (
            <section className="kpi-dashboard-footer" aria-label="สมมติฐานการอ่านตัวเลข">
              <article className="panel kpi-assumptions-panel">
                <div className="panel-title">
